...
EXTERNAL_URL="${EXTERNAL_URL:-http://192.168.2.226:8000}"

echo -n "Waiting for settings table"
for i in $(seq 1 30); do
  TABLE=$(docker exec integrated-postgres psql -U sub2api -d sub2api -tAc "SELECT 1 FROM pg_tables WHERE tablename='settings'" 2>/dev/null)
  if [ "$TABLE" = "1" ]; then
    echo
    break
  fi
...
docker exec -i integrated-postgres psql -U sub2api -d sub2api <<EOSQL
...
docker restart integrated-sub2api > /dev/null
sleep 3
